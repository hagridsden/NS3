...
int main (int argc, char *argv[])
{
  ...
  double interPacketInterval = 100; // in milliseconds
  double packetSize = 1000;			// MAx size = 1024 Bytes
  /*
...
*/
  ...
  p2ph.SetDeviceAttribute ("DataRate", DataRateValue (DataRate ("1Mbps")));		// P-to-P Bandwidth
...
//	uint16_t ulPort = 1235;
  ...
  //ApplicationContainer onOffApp;
	...
	ApplicationContainer serverApps;
	  ...
	  dlClient.SetAttribute ("MaxPackets", UintegerValue(500000));
	  dlClient.SetAttribute ("PacketSize", UintegerValue(packetSize));
	  UdpClientHelper ulClient (remoteHostAddr, ulPort);
	  ulClient.SetAttribute ("Interval", TimeValue (MilliSeconds(interPacketInterval)));
	  ulClient.SetAttribute ("MaxPackets", UintegerValue(500000));
...
//onOffApp.Stop(Seconds(0.1));
  */
  ...
  p2ph.EnablePcapAll("epc-try7");


  FlowMonitorHelper fmhelper;
  Ptr<FlowMonitor> flowMon = fmhelper.Install(ueNodes);
  flowMon = fmhelper.Install(remoteHost);
  ...
  flowMon->SetAttribute("PacketSizeBinWidth", DoubleValue (1000));
  ...
  flowMon->SerializeToXmlFile ("results_epc_try1.xml",false,false);  //  Print per flow statistics
